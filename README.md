...
> Gemini 사용 시 지원·권장 모델 예시: `gemini-1.5-flash`, `gemini-1.5-flash-001`, `gemini-1.5-pro`, `gemini-3-pro-preview`, `gemini-3.5-pro-preview`. UI에서 괄호가 포함된 라벨이나 `models/` 접두사가 붙어 있어도 앱이 자동으로 정규화해 올바른 엔드포인트로 요청합니다. 모델명이 잘못되면 404 안내 메시지와 함께 시도한 모델 목록을 표시합니다.
